...
- [数据结构简介](#数据结构简介)
- [调试redis 3.0源码](#调试redis-30源码)
- [ziplist数据结构](#ziplist数据结构)
- [hash](#hash)
- [set](#set)
...
好的，我会用 ASCII art 的方式为你描述 Redis 的主要数据结构。但请注意，由于 ASCII art 的局限性，这种表示方式将是一个非常简化的模型。
...
```
   +----------+         +---------------------+
   | FieldKey | ------> |   FieldValue (SDS)  |
   +----------+         +---------------------+
   |   ...    |         |       ...           |
   +----------+         +---------------------+
```

5. **有序集合 (Sorted Set) - 使用跳跃表和哈希表**
...
6. **位图 (Bitset) - 字符串的特殊用法**

```
+----------+         +--------------------------+
|   Key    | ------> |  01010101... (Bit Array) |
+----------+         +--------------------------+
```

7. **HyperLogLog - 使用特殊算法和数据结构**

```
+----------+         +-----------------------+
|   Key    | ------> |    HyperLogLog data   |
+----------+         +-----------------------+
```
